...
class Gff:
    def __init__(self, gff_file: Path) -> None:
        ...
        self.feats = [GffFeature(
            x["region"], x["feature_type"],
            x["start"], x["end"], x["strand"], x["attrs"]
            ...
            for line in f:
                if line.startswith("#"):
                    ...
                    if key not in meta_d:
                        meta_d[key] = " ".join(line_data) if line.startswith("#!") or "gff_version" == key else [line_data]
                    else:
